...
<p>
  ...
  <span class="code"> build and install via eclipse  </div>
  <span class="code"> git checkout upgrade-branch  </div>
  <span class="code"> build and install via eclipse  </div>

<p>
Now, the only issue with this kind of install & test is that the upgrade methods: <span class="code">onUpgrade(SQLiteDatabase database, int oldVersion,int newVersion)   </div> did not get called. So, my horrible sql was never executed. 
</p>
...
<p>
  ...
  <span class="code"> adb uninstall plasma.apk  </div>
  <span class="code"> adb install plasma-oldversion.apk </div>
  <span class="code"> adb install plasma-newversion.apk  </div>
