...
<p>
  ...
  {% highlight bash %} build and install via eclipse {% endhighlight %}
  {% highlight bash %} git checkout upgrade-branch {% endhighlight %}
  {% highlight bash %} build and install via eclipse {% endhighlight %}
</p>
<p>
Now, the only issue with this kind of install & test is that the upgrade methods: {% highlight java %} onUpgrade(SQLiteDatabase database, int oldVersion,int newVersion)  {% endhighlight %} did not get called. So, my horrible sql was never executed. 
</p>
...
<p>
  ...
  {% highlight bash %} adb uninstall plasma.apk {% endhighlight %}
  {% highlight bash %} adb install plasma-oldversion.apk {% endhighlight %}
  {% highlight bash %} adb install plasma-newversion.apk {% endhighlight %}
