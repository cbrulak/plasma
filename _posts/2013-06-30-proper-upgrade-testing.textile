...
<p>
Now, the only issue with this kind of install & test is that the upgrade methods: 
</p>
<p>
<pre class="prettyprint">onUpgrade(SQLiteDatabase database, int oldVersion,int newVersion)</pre> did not get called. So, my horrible sql was never executed. 
</p>
